...
def pldist(Xprime, Pprime, Qprime):
    return math.fabs((Qprime[1]-Pprime[1])*Xprime[0] -(Qprime[0]-Pprime[0])*Xprime[1] +Qprime[0]*Pprime[1] - Qprime[1]*Pprime[0])/math.sqrt((Qprime[1]-Pprime[1])**2 + (Qprime[0]-Pprime[0])**2)
...
def generateMorphingImage(img1, img2, L1, L2, alpha):
    ...
    resLine = []
    for i in lineVect:
        resLine.append([L1[i[0]][0]*alpha+L2[i[0]][0]*(1-alpha), L1[i[0]][1]*alpha+L2[i[0]][1]*(1-alpha), L1[i[1]][0]*alpha+L2[i[1]][0]*(1-alpha), L1[i[1]][1]*alpha+L2[i[1]][1]*(1-alpha)])
    h,w,c = img1.shape
    a=5
    b=0.9
    p=0
    ...
    for i in range(0,h):
        for j in range(0,w):
            ...
            vtmp = 0
            for k in range(0,len(lineVect)):
                x = findX([j, i], [resLine[k][0], resLine[k][1]], [resLine[k][2], resLine[k][3]], [L2[lineVect[k][0]][0], L2[lineVect[k][0]][1]], [L1[lineVect[k][1]][0], L1[lineVect[k][1]][1]])
                if x[0]>0 and x[0]<w and x[1]>0 and x[1]<h:
                    length = math.sqrt((resLine[k][2]-resLine[k][0])**2+(resLine[k][3]-resLine[k][1])**2)
                    dist = pldist([i,j], [resLine[k][0], resLine[k][1]], [resLine[k][2], resLine[k][3]])
                    weight.append((length**p/(a+dist))**b)
...
for i in range(1,10):
    ...
    cv2.imwrite("bn/img"+str(i)+".png", img3)
